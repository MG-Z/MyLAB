...
public class SimpleEventDispatcher implements IEventDispatcher {
    ...
    public void fireEvent(Event event) {
        ...
        if (listeners == null) {
            System.out.println(String.format("can not find event listener for eventId:%s", event.getEventId()));
            return;
        }
        for (IEventListener listener : listeners) {
            try {
                ...
                System.out.println("fire event error. event type :" + event.getEventId() + " event params :" + event.getParams().toString());
            }
        }
    }
